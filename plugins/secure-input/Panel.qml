...
Item {
  ...
  Rectangle {
    anchors.fill: parent
    radius: height / 2
    color: root.requests.length ? Color.urgent : "transparent"
    Text {
      anchors.centerIn: parent
      text: root.requests.length ? "!" : "●"
    ...
    Rectangle {
      ...
      Column {
        ...
        Row {
          spacing: 8
          visible: !!root.selected
          Rectangle { width: 100; height: 34; color: Color.accent; radius: 6; Text { anchors.centerIn: parent; text: "Autorizar"; color: Color.popups.text }; MouseArea { anchors.fill: parent; onClicked: root.approve() } }
          Rectangle { width: 100; height: 34; color: Color.urgent; radius: 6; Text { anchors.centerIn: parent; text: "Cancelar"; color: Color.popups.text }; MouseArea { anchors.fill: parent; onClicked: root.cancelRequest() } }
        }
      }
    }
  }
